...
class BirdCardBoard extends StatefulWidget {
  final Function() onWin;

  const BirdCardBoard({Key key, this.onWin}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return BirdCardBoardState();
  }
}

class BirdCardBoardState extends State<BirdCardBoard> {
  ...
  List<CardModel> createCards() {
    List<String> asset = [];
    List(8).forEach((f) => asset.add('B0${(asset.length + 1)}.png'));
    List(8).forEach((f) => asset.add('B0${(asset.length - 8 + 1)}.png'));
    return List(16).map((f) {
      int index = Random().nextInt(1000) % asset.length;
      String _image =
          'assets/B' + asset[index].substring(asset[index].length - 6);

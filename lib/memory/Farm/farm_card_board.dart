...
class FarmCardBoard extends StatefulWidget {
  final Function() onWin;

  const FarmCardBoard({Key key, this.onWin}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return FarmCardBoardState();
  }
}

class FarmCardBoardState extends State<FarmCardBoard> {
  ...
  List<CardModel> createCards() {
    List<String> asset = [];
    List(8).forEach((f) => asset.add('F0${(asset.length + 1)}.png'));
    List(8).forEach((f) => asset.add('F0${(asset.length - 8 + 1)}.png'));
    return List(16).map((f) {
      int index = Random().nextInt(1000) % asset.length;
      String _image =
          'assets/F' + asset[index].substring(asset[index].length - 6);
